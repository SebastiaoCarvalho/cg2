...
const lWaist = 10,
    hWaist = 5,
    dWaist = 5;
var waistMaterial = new THREE.MeshBasicMaterial({
    color: 0xffd700,
...
const lAbdomen = 4,
    hAbdomen = 3,
    dAbdomen = 5;
var abdomenChestArmForearmMaterial = new THREE.MeshBasicMaterial({
    color: 0xff0000,
    wireframe: wireframing,
});
...
const rWheel = 1.5,
    hWheel = 1;
var wheelTubeEyeConnectorMaterial = new THREE.MeshBasicMaterial({
    color: 0x000000,
    wireframe: wireframing,
});
...
const lHead = 4,
    hHead = 4,
    dHead = 3;
var headAntenaLegsFootMaterial = new THREE.MeshBasicMaterial({
    color: 0x0000ff,
    wireframe: wireframing,
});
...
const lThigh = 3,
    hThigh = 4,
    dThigh = 3;
var thighMaterial = new THREE.MeshBasicMaterial({
    color: 0xd3d3d3,
    wireframe: wireframing,
});
...
const lTrailer = 12,
    hTrailer = 18 - 5.5,
    dTrailer = 48;
var trailerMaterial = new THREE.MeshBasicMaterial({
    color: 0x838383,
    wireframe: wireframing,
});
...
/* Trailer Wheel */
const rTrailerWheel = 2,
    hTrailerWheel = 1;

/* Trailer Connector */
const rTrailerConnector = 1,
    hTrailerConnector = 2;
...
function createRobot(x, y, z) {
    ...
    addWheel(
        legs,
        lWaist / 2 + hWheel / 2,
        -hWaist / 2 - hThigh - hLeg + 2 + 2 * rWheel + 2 + rWheel,
        dLeg / 2
    );
    addWheel(
        legs,
...
function createTrailer(x, y, z) {
    ...
    addTrailerBase(
        trailer,
        0,
        -hTrailer / 2 - hTrailerBase / 2,
        -dTrailer / 2 + dTrailerBase / 2
    );

    addTWheel(
        trailer,
        lTrailer / 2,
        -hTrailer / 2 - hTrailerBase,
        -dTrailer / 2 + 2 + rTrailerWheel
    );
    addTWheel(
        trailer,
        lTrailer / 2,
        -hTrailer / 2 - hTrailerBase,
        -dTrailer / 2 + 2 + 2 * rTrailerWheel + 2 + rTrailerWheel
    );
    addTWheel(
        trailer,
        -lTrailer / 2,
    ...
    addConnector(
        trailer,
        0,
        -hTrailer / 2 - hTrailerConnector / 2,
        dTrailer / 2 - 3 * rTrailerConnector
    );
...
function checkCollisions() {
    ...
    const truckYMax =
        robot.position.y + hWaist / 2 + +hAbdomen + hChest + margin;
    ...
    const xMatch =
        (trailerXMin < truckXMin && truckXMin < trailerXMax) ||
        (trailerXMin < truckXMax && truckXMax < trailerXMax);
    const yMatch =
        (trailerYMin < truckYMin && truckYMin < trailerYMax) ||
        (trailerYMin < truckYMax && truckYMax < trailerYMax);
    const zMatch =
        (trailerZMin < truckZMin && truckZMin < trailerZMax) ||
        (trailerZMin < truckZMax && truckZMax < trailerZMax);
...
function update() {
    ...
    if (!isCollision) {
        if (
            (leftArrowPressed ? !rightArrowPressed : rightArrowPressed) &&
            (upArrowPressed ? !downArrowPressed : downArrowPressed)
        )
            // normalize if move is diagonal
            velocityValue =
                (velocityValue /
...
function init() {
    ...
    materials = [
        waistMaterial,
        abdomenChestArmForearmMaterial,
        headAntenaLegsFootMaterial,
        thighMaterial,
        trailerMaterial,
        wheelTubeEyeConnectorMaterial,
    ];
